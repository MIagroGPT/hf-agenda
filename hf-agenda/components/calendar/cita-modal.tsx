...
export function CitaModal({ open, onClose, cita, defaultDate, onSuccess }: CitaModalProps) {
  const [form, setForm] = useState({
    ...
    barberoId: "",
    servicioId: "",
    inicio: defaultDate ? defaultDate.slice(0, 16) : "",
    fin: "",
    notas: "",
    precio: "",
    estado: "PENDIENTE",
  ...
  useEffect(() => {
    if (cita) {
      setForm({
        ...
        inicio: cita.inicio?.slice(0, 16) || "",
        fin: cita.fin?.slice(0, 16) || "",
        notas: cita.notas || "",
      ...
      setForm((f) => ({
        ...
        notas: "", precio: "", estado: "PENDIENTE",
      }));
    }
  }, [cita, defaultDate]);

  // Auto-calcular fin cuando se selecciona servicio
  useEffect(() => {
    if (form.servicioId && form.inicio) {
      const servicio = servicios.find((s: any) => s.id === form.servicioId);
      if (servicio) {
        const inicioDate = new Date(form.inicio);
        const finDate = new Date(inicioDate.getTime() + servicio.duracion * 60000);
        setForm((f) => ({ ...f, fin: finDate.toISOString().slice(0, 16) }));
        if (!form.precio) setForm((f) => ({ ...f, precio: servicio.precio.toString() }));
      }
    }
  }, [form.servicioId, form.inicio]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-[#1A1A1A] border border-white/10 rounded-2xl shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-neutral-400 mb-1.5">Precio</label>
              <input
                type="number"
                value={form.precio}
